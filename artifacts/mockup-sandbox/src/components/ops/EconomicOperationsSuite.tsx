...
import { economicOperationsClient, toOperationalState, type CommandCenterDto, type ReplayDto, type RollbackDto, type TimelineDto } from "@/lib/economic-operations-client";
...
export const ProofExplorer = ({ proofs }: { proofs: Array<{ title: string; summary: string; details: string }> }) => <div className="space-y-3">{proofs.map((p) => <ProofDrawer key={p.title} title={p.title} summary={p.summary}>{p.details}</ProofDrawer>)}</div>;
...
export function ExecutiveOpsRuntime() {
  ...
  useEffect(() => { if (!selectedId) return; Promise.allSettled([economicOperationsClient.getTimeline(selectedId), economicOperationsClient.getReplay(selectedId), economicOperationsClient.getRollback(selectedId)]).then(([t, rp, rb]) => { if (t.status === "fulfilled") setTimeline(t.value); if (rp.status === "fulfilled") setReplay(rp.value); if (rb.status === "fulfilled") setRollback(rb.value); }); }, [selectedId]);

  const proofItems = useMemo(() => [{ title: "Savings evidence", summary: "Invoice delta and assignment lineage", details: "Verified from invoice deltas and tenant-level reconciliation." }, { title: "Approval log", summary: "Policy, approvers, and timing", details: "Approval required for staged deprovisioning under ECON-42." }], []);
  ...
  return <div className="space-y-6"><EconomicCommandCenter data={data} onSelect={setSelectedId} selectedId={selectedId} /><section className="grid gap-6 xl:grid-cols-5"><div className="space-y-2 xl:col-span-3"><h2 className="text-lg font-semibold">Execution Timeline Explorer</h2><ExecutionTimelineExplorer timeline={timeline} selectedEvent={selectedEvent} onSelectEvent={setSelectedEvent} /></div><div className="space-y-2 xl:col-span-2"><h2 className="text-lg font-semibold">Operations Health Panel</h2><OperationsHealthPanel data={data} /></div></section><section className="grid gap-6 lg:grid-cols-2"><div><h2 className="text-lg font-semibold mb-2">Proof Explorer</h2><ProofExplorer proofs={proofItems} /></div><div className="space-y-3"><h2 className="text-lg font-semibold">Simulation Preview</h2><SimulationPreview replay={replay} /><RollbackExplorer rollback={rollback} /><SavingsEvidencePanel value="$118,200 / year" proof="Savings evidence recorded and drift monitoring active." /></div></section><section className="grid gap-4 lg:grid-cols-3"><ConnectorReadinessPanel data={data} /><ApprovalQueuePanel data={data} /><VerdictCard title="Tenant posture" state={toOperationalState(data.tenantPosture[0]?.state ?? "MANUAL_ONLY")} impact="Tenant isolation enforced" action="Resolve connector drift before autonomous execution"><TenantPosturePanel data={data} /></VerdictCard></section></div>;
}
