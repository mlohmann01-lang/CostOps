...
import { getCertifiedWedgeRegistrySummary } from "../lib/certification/certified-wedge-registry";
...
async function certifiedWedges(tenantId: string) { const [m365, ai, servicenow, dataPlatform, aws, azure, itam] = await Promise.all([getM365WedgeCertification(tenantId), getAIWedgeCertification(tenantId), getServiceNowWedgeCertification(tenantId), getDataPlatformWedgeCertification(tenantId), getAwsWedgeCertification(tenantId), getAzureWedgeCertification(tenantId), getItamWedgeCertification(tenantId)]); return { m365: Boolean((m365 as any).certified), ai: (ai.certifiedAssets ?? 0) > 0 && ai.uncertifiedAssets === 0, servicenow: Boolean(servicenow.certified), aws: Boolean(aws.certified), azure: Boolean(azure.certified), snowflake: Boolean(dataPlatform.snowflakeCertified), databricks: Boolean(dataPlatform.databricksCertified), dataPlatform: Boolean(dataPlatform.certified), itam: Boolean(itam.certified) }; }

router.get("/live-tenant-readiness", async (req, res) => { try { if (!guarded(req, res, "READ")) return; const tenantId = tenant(req); const health = getConnectorHealthDashboard(tenantId); const [wedges, registrySummary] = await Promise.all([certifiedWedges(tenantId), getCertifiedWedgeRegistrySummary(tenantId)]); const dashboard = await getLiveTenantReadinessDashboard(tenantId, wedges, health); return res.json({ ...dashboard, certifiedWedgeRegistry: { totalWedges: registrySummary.totalWedges, certifiedWedges: registrySummary.certifiedWedges, productionReadyWedges: registrySummary.productionReadyWedges, blockers: registrySummary.blockers } }); } catch (error) { return res.status(409).json(classifyProductionError(error)); } });
