...
async function certifiedWedges(tenantId: string) { const [m365, ai, servicenow] = await Promise.all([getM365WedgeCertification(tenantId), getAIWedgeCertification(tenantId), getServiceNowWedgeCertification(tenantId)]); return { m365: Boolean((m365 as any).certified), ai: (ai.certifiedAssets ?? 0) > 0 && ai.uncertifiedAssets === 0, servicenow: Boolean(servicenow.certified) }; }
