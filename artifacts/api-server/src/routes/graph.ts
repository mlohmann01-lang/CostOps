...
router.get("/entities/:id", async (req,res)=>{ const tenantId=(req.query.tenantId as string) ?? "default"; const rows = await db.select().from(operationalEntitiesTable).where(and(eq(operationalEntitiesTable.id, Number(req.params.id)), eq(operationalEntitiesTable.tenantId, tenantId))).limit(1); res.json(rows[0] ?? null); });
