...
router.get("/:id/outcome-correlation", async (req, res) => { const rows = await db.select().from(policySimulationsTable).where(eq(policySimulationsTable.id, Number(req.params.id))).limit(1); if (!rows[0]) return res.status(404).json({ error: "Not found" }); const r:any=rows[0]; return res.json({ simulationId: r.id, tenantId: r.tenantId, simulationProjectedSavings: Number(r.projectedMonthlySavings ?? 0), simulationProjectedConfidence: r.predictedRealizationConfidence, simulationBlastRadius: Number(r.blastRadiusScore ?? 0), simulationGovernanceRisk: Number(r.governanceRiskScore ?? 0) }); });
