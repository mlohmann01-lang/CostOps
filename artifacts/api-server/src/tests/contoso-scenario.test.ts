...
test("Contoso Scenario Integration Test Matrix V1", async () => {
  ...
  const approved = await runExecutionEngine({ recommendation: { ...recommendations[0], executionStatus: "APPROVAL_REQUIRED", criticalBlockers: [], actionRiskProfile: { riskClass: "B" }, approvalStatus: "APPROVED", approvalEscalated: true }, actorId: "approver@contoso.com", tenantId: "default", mode: "APPROVAL_EXECUTE", mvpMode: true });
