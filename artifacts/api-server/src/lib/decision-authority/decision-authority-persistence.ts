import type { DecisionCollection } from './decision-authority-types';
...
export class MemoryPersistenceStore<T extends Keyed> implements Store<T> {
  ...
  constructor(readonly collection: DecisionCollection) {}
...
export class DatabasePersistenceStore<T extends Keyed> implements Store<T> {
  ...
  async upsert(v: T) {
    const { db, decisionAuthorityRecordsTable } = await this.db();
    await db.insert(decisionAuthorityRecordsTable).values({ id: this.pk(v), tenantId: v.tenantId, collection: this.collection, recordId: v.id, payload: v as any })
      .onConflictDoUpdate({ target: decisionAuthorityRecordsTable.id, set: { payload: v as any, updatedAt: new Date() } });
    return v;
  }
  async get(t: string, id: string) {
    const { db, decisionAuthorityRecordsTable } = await this.db();
    const { and, eq } = await import('drizzle-orm');
    const rows = await db.select().from(decisionAuthorityRecordsTable).where(and(eq(decisionAuthorityRecordsTable.tenantId, t), eq(decisionAuthorityRecordsTable.collection, this.collection), eq(decisionAuthorityRecordsTable.recordId, id))).limit(1);
    return rows[0]?.payload as T | undefined;
  }
  async list(t: string, f: Record<string, unknown> = {}) {
    const { db, decisionAuthorityRecordsTable } = await this.db();
    const { and, eq } = await import('drizzle-orm');
    const rows = await db.select().from(decisionAuthorityRecordsTable).where(and(eq(decisionAuthorityRecordsTable.tenantId, t), eq(decisionAuthorityRecordsTable.collection, this.collection)));
    return rows.map((r) => r.payload as T).filter((v) => matches(v as any, f));
  }
  async deleteTenant(t: string) {
    const { db, decisionAuthorityRecordsTable } = await this.db();
    const { and, eq } = await import('drizzle-orm');
    await db.delete(decisionAuthorityRecordsTable).where(and(eq(decisionAuthorityRecordsTable.tenantId, t), eq(decisionAuthorityRecordsTable.collection, this.collection)));
  }
  async size() {
    const { db, decisionAuthorityRecordsTable } = await this.db();
    const { eq } = await import('drizzle-orm');
    return (await db.select().from(decisionAuthorityRecordsTable).where(eq(decisionAuthorityRecordsTable.collection, this.collection))).length;
  }
}

export function createPersistenceStore<T extends Keyed>(collection: DecisionCollection): Store<T> {
