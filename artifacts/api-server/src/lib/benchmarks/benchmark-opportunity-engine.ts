...
export function generateBenchmarkOpportunities(benchmarks: Benchmark[]): BenchmarkOpportunity[] {
  return benchmarks.filter((benchmark) => benchmark.variancePercent < 0).map((benchmark) => ({ id: `opp-benchmark-${benchmark.id}`, tenantId: benchmark.tenantId, source: "BENCHMARK", benchmarkId: benchmark.id, title: titleFor(benchmark), description: `${benchmark.category.replaceAll("_", " ")} is ${Math.abs(benchmark.variancePercent)} points below peer benchmark.`, domain: domainFor(benchmark), projectedMonthlySavings: Math.round(benchmarkRecoverableValue(benchmark) / 12), projectedAnnualSavings: benchmarkRecoverableValue(benchmark), trustScore: benchmark.impactLevel === "HIGH" ? 78 : 72, confidenceScore: benchmark.impactLevel === "HIGH" ? 84 : 76, urgency: benchmark.impactLevel === "HIGH" ? "HIGH" : benchmark.impactLevel === "MEDIUM" ? "MEDIUM" : "LOW", readiness: benchmark.impactLevel === "HIGH" ? "APPROVAL_REQUIRED" : "ELIGIBLE", sourceReferenceId: benchmark.id, status: "DISCOVERED", createdAt: benchmark.createdAt, updatedAt: new Date().toISOString(), variancePercent: benchmark.variancePercent }));
}
