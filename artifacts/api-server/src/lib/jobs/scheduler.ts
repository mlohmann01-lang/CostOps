...
export async function runDueJobs(){ const due=await getDueJobs(new Date()); const results=[] as any[]; for(const job of due){ const [running]=await db.select().from(jobRunsTable).where(and(eq(jobRunsTable.tenantId,job.tenantId),eq(jobRunsTable.jobType,job.jobType),eq(jobRunsTable.status,"RUNNING"))).limit(1); if(running){ results.push({jobId:job.id,status:"SKIPPED",evidence:{reason:"DUPLICATE_RUNNING"}}); continue; }
const r=await runJob({tenantId:job.tenantId,jobType:job.jobType,scheduledJobId:job.id,payload:{},failedCount:(job as any).failureCount??0,connectorStatus:(job as any).connectorStatus}); await db.update(scheduledJobsTable).set({lastRunAt:new Date(),nextRunAt:calculateNextRun(job.scheduleConfig),status:r.ok?"SUCCEEDED":"FAILED"}).where(eq(scheduledJobsTable.id,job.id)); results.push({jobId:job.id,status:r.ok?"SUCCEEDED":"FAILED"}); }
 return results; }
