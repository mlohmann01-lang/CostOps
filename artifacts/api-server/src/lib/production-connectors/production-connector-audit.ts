...
export async function runM365EntraLiveIntegrationAudit() {
  ...
  const dryRun = await runner.dryRun("audit", "m365", { tokenProvider: async () => "token" });
  ...
  const sync = await runner.sync("audit", "m365", { tokenProvider: async () => "token", authorised: true, liveTenantReady: true });
