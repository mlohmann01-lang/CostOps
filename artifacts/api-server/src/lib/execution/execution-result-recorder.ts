...
export class ExecutionResultRecorder {
  ...
  async create(row: typeof executionResultsTable.$inferInsert){
    ...
    catch(e){ if(!allowMemory()) throw new Error(`EXECUTION_RESULT_DB_UNAVAILABLE_FAIL_CLOSED: ${(e as Error).message}`); const saved={id:Date.now(),createdAt:new Date(),...row}; ExecutionResultRecorder.mem.set(this.key(row.tenantId,row.executionRequestId),saved); return saved; }
  }
