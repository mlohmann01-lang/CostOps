import type { AIValueAttributionCollection } from './ai-value-attribution-types';
...
export class MemoryPersistenceStore<T extends Keyed> implements Store<T> {
  ...
  constructor(readonly collection: AIValueAttributionCollection) {}
...
export class DatabasePersistenceStore<T extends Keyed> implements Store<T> {
  ...
  async upsert(v: T) {
    const { db, aiValueAttributionRecordsTable } = await this.db();
    await db.insert(aiValueAttributionRecordsTable).values({ id: this.pk(v), tenantId: v.tenantId, collection: this.collection, recordId: v.id, payload: v as any })
      .onConflictDoUpdate({ target: aiValueAttributionRecordsTable.id, set: { payload: v as any, updatedAt: new Date() } });
    return v;
  }
  async get(t: string, id: string) {
    const { db, aiValueAttributionRecordsTable } = await this.db();
    const { and, eq } = await import('drizzle-orm');
    const rows = await db.select().from(aiValueAttributionRecordsTable).where(and(eq(aiValueAttributionRecordsTable.tenantId, t), eq(aiValueAttributionRecordsTable.collection, this.collection), eq(aiValueAttributionRecordsTable.recordId, id))).limit(1);
    return rows[0]?.payload as T | undefined;
  }
  async list(t: string, f: Record<string, unknown> = {}) {
    const { db, aiValueAttributionRecordsTable } = await this.db();
    const { and, eq } = await import('drizzle-orm');
    const rows = await db.select().from(aiValueAttributionRecordsTable).where(and(eq(aiValueAttributionRecordsTable.tenantId, t), eq(aiValueAttributionRecordsTable.collection, this.collection)));
    return rows.map((r) => r.payload as T).filter((v) => matches(v as any, f));
  }
  async deleteTenant(t: string) {
    const { db, aiValueAttributionRecordsTable } = await this.db();
    const { and, eq } = await import('drizzle-orm');
    await db.delete(aiValueAttributionRecordsTable).where(and(eq(aiValueAttributionRecordsTable.tenantId, t), eq(aiValueAttributionRecordsTable.collection, this.collection)));
  }
  async size() {
    const { db, aiValueAttributionRecordsTable } = await this.db();
    const { eq } = await import('drizzle-orm');
    return (await db.select().from(aiValueAttributionRecordsTable).where(eq(aiValueAttributionRecordsTable.collection, this.collection))).length;
  }
}

export function createPersistenceStore<T extends Keyed>(collection: AIValueAttributionCollection): Store<T> {
