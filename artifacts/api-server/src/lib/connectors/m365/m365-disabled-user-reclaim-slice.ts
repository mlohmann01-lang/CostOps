...
import { generateM365Recommendations, type M365OperationalRecommendation, type M365SkuPricing } from "./m365-recommendation-generator";

export class M365DisabledUserReclaimSliceService {
  constructor(private readonly syncService: M365ReadOnlyEvidenceSyncService) {}

  private deterministicCorrelationId(tenantId: string, recommendation: M365OperationalRecommendation): string {
    return createHash("sha256").update([tenantId, recommendation.playbookId, recommendation.affectedUser.userId, recommendation.affectedLicenses.join(",")].join("|")).digest("hex").slice(0, 32);
  }
  ...
  async run(input: { tenantId: string; tenantMode: string; skuPricingCatalog?: M365SkuPricing[] }) {
    ...
    const reclaimOnly = generated.recommendations;
    ...
    for (const rec of reclaimOnly) {
      ...
      if (input.tenantMode !== "DEMO" && rec.affectedUser.userPrincipalName.includes("demo")) continue;
      const [inserted] = await db.insert(recommendationsTable).values({
        ...
        trustScore: rec.trustLevel === "HIGH" ? 0.95 : rec.trustLevel === "MEDIUM" ? 0.75 : 0.4,
        executionStatus: "APPROVAL_REQUIRED",
        playbook: rec.playbookId,
        playbookId: rec.playbookId,
        playbookName: rec.playbookId,
        playbookEvidence: { proofReferences: rec.proofReferences, evidenceFreshness: rec.evidenceFreshness, evidenceConfidence: rec.evidenceConfidence, confidenceReasoning: rec.confidenceReasoning, utilizationReasoning: rec.utilizationReasoning },
        connector: "M365",
        ...
        rollbackNotes: rec.rollbackSupport ? "HIGH" : "LOW",
      ...
      const [ledger] = await db.insert(outcomeLedgerTable).values({
        ...
        savingConfidence: rec.trustLevel,
