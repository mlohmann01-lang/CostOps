...
import { candidate, daysSince, lastActivityFor, latestSnapshotOrThrow, monthlySkuCost } from './m365-playbook-utils'
...
export class LicensePoolRecoveryPlaybook implements M365Playbook {
  ...
  async evaluate(tenantId: string, snapshotId: string) {
    ...
    return snapshot.skus.flatMap((sku) => {
      ...
      const monthly = recoverable * monthlySkuCost(sku.skuPartNumber)
