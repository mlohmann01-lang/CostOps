...
export class DatabaseMicrosoftCredentialStore implements MicrosoftCredentialBackingStore {
  ...
  // Tenant-scoped at the query level (not just filtered in-process after fetch): a
  // credentialRef belonging to a different tenant never returns a row.
  async get(tenantId: string, credentialRef: string): Promise<StoredMicrosoftCredential | undefined> {
    const { db, microsoftOauthCredentialsTable } = await this.db();
    const { and, eq } = await import("drizzle-orm");
    const rows = await db
      .select()
      .from(microsoftOauthCredentialsTable)
      .where(and(eq(microsoftOauthCredentialsTable.credentialRef, credentialRef), eq(microsoftOauthCredentialsTable.tenantId, tenantId)))
      .limit(1);
    const row = rows[0];
