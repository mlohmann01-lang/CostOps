...
export class ApprovalAuthorityService {
  ...
  async submitForApproval(tenantId: string, targetType: ApprovalAuthorityTargetType, targetId: string, input: SubmitInput = {}) {
    ...
    if (targetType === "RECOMMENDATION") {
      ...
      const governanceEvent = await this.recommendationEvents.emit({ tenantId, recommendationId: targetId, eventType: "RECOMMENDATION_SUBMITTED_FOR_APPROVAL", actorId, actorRole, eventReason: input.reason ?? "Submitted to approval workflow", beforeState: String((recommendation as any)?.recommendationState ?? ""), afterState: String((linked as any).recommendationState), beforeReadiness: String((recommendation as any)?.executionReadiness ?? ""), afterReadiness: String((linked as any).executionReadiness), evidenceSnapshot: evidence, approvalSnapshot: { workflowId: workflow.workflowId, approvalState: workflow.approvalState, currentStage, sourceSystem: "APPROVAL_WORKFLOW" } });
      appendUnifiedEvent(normalizeRecommendationGovernanceEvent({ ...(governanceEvent as any), id: `${targetId}:RECOMMENDATION_SUBMITTED_FOR_APPROVAL:governance:${workflow.workflowId}`, tenantId, recommendationId: targetId, eventType: "RECOMMENDATION_SUBMITTED_FOR_APPROVAL", actorId, actorRole, eventReason: input.reason ?? "Submitted to approval workflow", beforeState: String((recommendation as any)?.recommendationState ?? ""), afterState: String((linked as any).recommendationState), evidenceSnapshot: evidence, createdAt: workflow.createdAt }));
