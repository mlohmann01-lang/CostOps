...
function verifyRouteToRepositoryBoundary(): DatabaseTenantIsolationDomainResult {
  ...
  const ev: DatabaseTenantIsolationEvidence[] = [
    ...
    evidence(domain, 'rtb-ev-2', 'ROUTE_BOUNDARY', 'src/routes/evidence-registry.ts', 'tenant() helper reads the guard-validated `req.tenantId` first (previously read `req.__authContext?.tenantId` first, which for a PLATFORM_ADMIN acting cross-tenant would resolve to the admin\'s own tenant rather than the requested target — fixed alongside the same bug found in ai-capital-allocation.ts and 8 other route files), with header/query/body fallbacks; write handlers build `{...req.body, tenantId: tenant(req)}` — tenantId spread LAST, so it cannot be overridden by a client-supplied tenantId in the body.', true, 0.65, 'tenant'),
