import { validateJwtToken } from "./jwt-validation.js";
...
export async function exchangeCodeForClaims(code: string): Promise<ProviderClaims> {
  ...
  // TODO: Exchange authorization code for tokens via Entra token endpoint.
  // Currently uses validateJwtToken as a placeholder — replace with real token exchange.
  const result = await validateJwtToken("mock");
