import type { AIInitiativePortfolioCollection } from './ai-initiative-portfolio-types';
...
export class MemoryPersistenceStore<T extends Keyed> implements Store<T> {
  ...
  constructor(readonly collection: AIInitiativePortfolioCollection) {}
...
export class DatabasePersistenceStore<T extends Keyed> implements Store<T> {
  ...
  async upsert(v: T) {
    const { db, aiInitiativePortfolioRecordsTable } = await this.db();
    await db.insert(aiInitiativePortfolioRecordsTable).values({ id: this.pk(v), tenantId: v.tenantId, collection: this.collection, recordId: v.id, payload: v as any })
      .onConflictDoUpdate({ target: aiInitiativePortfolioRecordsTable.id, set: { payload: v as any, updatedAt: new Date() } });
    return v;
  }
  async get(t: string, id: string) {
    const { db, aiInitiativePortfolioRecordsTable } = await this.db();
    const { and, eq } = await import('drizzle-orm');
    const rows = await db.select().from(aiInitiativePortfolioRecordsTable).where(and(eq(aiInitiativePortfolioRecordsTable.tenantId, t), eq(aiInitiativePortfolioRecordsTable.collection, this.collection), eq(aiInitiativePortfolioRecordsTable.recordId, id))).limit(1);
    return rows[0]?.payload as T | undefined;
  }
  async list(t: string, f: Record<string, unknown> = {}) {
    const { db, aiInitiativePortfolioRecordsTable } = await this.db();
    const { and, eq } = await import('drizzle-orm');
    const rows = await db.select().from(aiInitiativePortfolioRecordsTable).where(and(eq(aiInitiativePortfolioRecordsTable.tenantId, t), eq(aiInitiativePortfolioRecordsTable.collection, this.collection)));
    return rows.map((r) => r.payload as T).filter((v) => matches(v as any, f));
  }
  async deleteTenant(t: string) {
    const { db, aiInitiativePortfolioRecordsTable } = await this.db();
    const { and, eq } = await import('drizzle-orm');
    await db.delete(aiInitiativePortfolioRecordsTable).where(and(eq(aiInitiativePortfolioRecordsTable.tenantId, t), eq(aiInitiativePortfolioRecordsTable.collection, this.collection)));
  }
  async size() {
    const { db, aiInitiativePortfolioRecordsTable } = await this.db();
    const { eq } = await import('drizzle-orm');
    return (await db.select().from(aiInitiativePortfolioRecordsTable).where(eq(aiInitiativePortfolioRecordsTable.collection, this.collection))).length;
  }
}

export function createPersistenceStore<T extends Keyed>(collection: AIInitiativePortfolioCollection): Store<T> {
