import type { AIEconomicsCollection } from './ai-economics-types';
...
export class MemoryPersistenceStore<T extends Keyed> implements Store<T> {
  ...
  constructor(readonly collection: AIEconomicsCollection) {}
...
export class DatabasePersistenceStore<T extends Keyed> implements Store<T> {
  ...
  async upsert(v: T) {
    const { db, aiEconomicsRecordsTable } = await this.db();
    await db.insert(aiEconomicsRecordsTable).values({ id: this.pk(v), tenantId: v.tenantId, collection: this.collection, recordId: v.id, payload: v as any })
      .onConflictDoUpdate({ target: aiEconomicsRecordsTable.id, set: { payload: v as any, updatedAt: new Date() } });
    return v;
  }
  async get(t: string, id: string) {
    const { db, aiEconomicsRecordsTable } = await this.db();
    const { and, eq } = await import('drizzle-orm');
    const rows = await db.select().from(aiEconomicsRecordsTable).where(and(eq(aiEconomicsRecordsTable.tenantId, t), eq(aiEconomicsRecordsTable.collection, this.collection), eq(aiEconomicsRecordsTable.recordId, id))).limit(1);
    return rows[0]?.payload as T | undefined;
  }
  async list(t: string, f: Record<string, unknown> = {}) {
    const { db, aiEconomicsRecordsTable } = await this.db();
    const { and, eq } = await import('drizzle-orm');
    const rows = await db.select().from(aiEconomicsRecordsTable).where(and(eq(aiEconomicsRecordsTable.tenantId, t), eq(aiEconomicsRecordsTable.collection, this.collection)));
    return rows.map((r) => r.payload as T).filter((v) => matches(v as any, f));
  }
  async deleteTenant(t: string) {
    const { db, aiEconomicsRecordsTable } = await this.db();
    const { and, eq } = await import('drizzle-orm');
    await db.delete(aiEconomicsRecordsTable).where(and(eq(aiEconomicsRecordsTable.tenantId, t), eq(aiEconomicsRecordsTable.collection, this.collection)));
  }
  async size() {
    const { db, aiEconomicsRecordsTable } = await this.db();
    const { eq } = await import('drizzle-orm');
    return (await db.select().from(aiEconomicsRecordsTable).where(eq(aiEconomicsRecordsTable.collection, this.collection))).length;
  }
}

export function createPersistenceStore<T extends Keyed>(collection: AIEconomicsCollection): Store<T> {
