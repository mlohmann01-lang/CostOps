...
const arch = spawnSync(process.execPath, ['--test', 'artifacts/api-server/src/tests/architecture-boundaries.test.ts'], { stdio: 'inherit', cwd: '/workspace/CostOps', env: { ...process.env, DATABASE_URL: process.env.DATABASE_URL ?? 'postgres://localhost:5432/contoso_test' } });
