...
export type CommandDataState = 'LIVE' | 'DEMO' | 'NOT_CONNECTED' | 'NO_DATA'
...
export function useCommandData(): any {
  ...
  return useMemo(() => {
    if (workspace.mode === 'demo') return { loading: false, error: null, refresh: () => Promise.resolve({ actions: demo.actions, metrics: demo.commandMetrics, posture: demo.posture, priority: demo.priority }), isEmptyLive: false, dataState: 'DEMO' as CommandDataState, data: { actions: demo.actions, metrics: demo.commandMetrics, posture: demo.posture, priority: demo.priority } }
    if (!workspace.dataReady) return { loading: false, error: null, refresh: live.refresh, isEmptyLive: true, dataState: 'NOT_CONNECTED' as CommandDataState, data: emptyCommand }
    const dataState: CommandDataState = live.error ? 'NO_DATA' : live.isEmpty ? 'NO_DATA' : 'LIVE'
