...
export const emptyCommandMetrics = { totalIdentified: 0, eligibleNow: 0, pendingApproval: 0, blockedManual: 0, verifiedSavings: 0, pendingVerification: 0, failedVerification: 0, activeDrift: 0, projectedValuePendingProof: 0 }
...
export const emptyOutcomes = { stats: [] as number[], ledger: [] as any[], unverified: { count: 0, verificationFailures: 0, projectedValuePendingProof: 0 } as any }
...
export function normalizeOutcomes(payload: unknown) {
  ...
  const unverifiedPayload = Array.isArray(source) ? source[2] : source?.unverified
  const ledger = rows(ledgerPayload, ['ledger', 'outcomes']).map((row: any) => {
    const evidencePack = row?.evidencePack ?? row?.evidence_pack ?? row?.evidence
    ...
    const status = s(row?.verificationStatus ?? row?.state ?? row?.status, verifiedValue ? 'Verified' : 'Pending')
    return { id: s(row?.id ?? row?.outcomeId, 'unknown'), action: s(row?.action ?? row?.title ?? row?.name, 'Untitled outcome'), projected: n(row?.projected ?? row?.projectedMonthlySavings ?? row?.monthlySaving), verified: verifiedValue, variance: row?.variance === null || row?.savingsVariance === null ? null : n(row?.variance ?? row?.savingsVariance), confidence: s(row?.confidence ?? row?.verificationConfidence, 'LOW'), evidence: s(row?.evidence === 'Available' ? 'Available' : row?.evidenceSummary, evidencePack ? 'Available' : verifiedValue ? 'Evidence-backed' : '—'), state: status, verificationStatus: status, verificationAge: s(row?.verificationAge?.label ?? row?.verificationAge, ''), evidencePack }
  ...
  const pending = n(unverifiedPayload?.count ?? summaryPayload?.pending ?? summaryPayload?.verificationPending ?? summaryPayload?.pendingVerificationCount ?? ledger.filter((row: any) => !String(row.verificationStatus).toUpperCase().includes('VERIFIED')).length)
  const failed = n(unverifiedPayload?.verificationFailures ?? summaryPayload?.failed ?? summaryPayload?.failedVerification ?? summaryPayload?.failedVerificationCount ?? ledger.filter((row: any) => String(row.verificationStatus).toUpperCase().includes('FAILED')).length)
  return { stats: [projected, verified, verified - projected, pending, failed, n(summaryPayload?.driftDetected ?? summaryPayload?.activeDrift)], ledger, unverified: unverifiedPayload ?? { count: pending, verificationFailures: failed, projectedValuePendingProof: ledger.filter((row: any) => !String(row.verificationStatus).toUpperCase().includes('VERIFIED')).reduce((sum: number, row: any) => sum + n(row.projected), 0) } }
}
