...
export default function LiveTenantReadinessView() {
  ...
  return <Shell><div style={{ padding: 24, display: 'grid', gap: 18 }}>
    ...
    <section style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(0, 1fr))', gap: 10 }}>
      <MetricCard label='Certified Wedges' value={String(Object.values(readiness.certifiedWedges).filter(Boolean).length)} delta='M365 · AI · ServiceNow' />
    ...
    <Card><SectionLabel>Certified Wedges</SectionLabel><div style={{ display: 'flex', gap: 10, marginTop: 10 }}><span>M365 {readiness.certifiedWedges.m365 ? <Badge label='Certified' /> : <Badge label='Missing' tone='red' />}</span><span>AI {readiness.certifiedWedges.ai ? <Badge label='Certified' /> : <Badge label='Missing' tone='red' />}</span><span>ServiceNow {readiness.certifiedWedges.servicenow ? <Badge label='Certified' /> : <Badge label='Missing' tone='red' />}</span></div></Card>
