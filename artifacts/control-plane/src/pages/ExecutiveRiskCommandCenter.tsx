...
export default function ExecutiveRiskCommandCenter() {
  ...
  const exposedSpend = data.domainBreakdown.find((domain) => domain.domain === 'RENEWALS')?.exposedSpend
  const actionMap = new Map(data.leadershipActions.map((action) => [action.label, action]))
  const actionLabels = data.dataState === 'LIVE' || data.dataState === 'DEMO'
    ? actionOrder.map((action, index) => actionMap.get(action) ?? { label: action, count: index < 4 ? 1 : 0, priority: index < 3 ? 'HIGH' : 'MEDIUM', actionType: action.toUpperCase().replace(/ /g, '_'), rationale: 'Leadership action for executive review.' })
    : []
  ...
  return <Shell><main style={{ padding:'18px clamp(16px, 2.4vw, 28px)', display:'grid', gap:14, maxWidth:1480, margin:'0 auto', width:'100%', boxSizing:'border-box' }}>
    ...
    {data.dataState !== 'LIVE' && <DataStateBanner state={data.dataState} detail={data.error} ctaLabel={data.dataState === 'NOT_CONNECTED' ? 'Connect Tenant' : undefined} ctaHref={data.dataState === 'NOT_CONNECTED' ? '/connectors' : undefined} />}
    ...
    <section data-testid='executive-narrative' style={{ border:'1px solid rgba(45,212,191,.22)', background:'rgba(45,212,191,.06)', borderRadius:16, padding:'13px 15px', color:'var(--text-secondary)', lineHeight:1.55 }}>
      <strong style={{ color:'var(--text-primary)' }}>Executive narrative:</strong> {data.executiveNarrative || executiveNarrative}
    </section>
    ...
    <div style={{ display:'grid', gridTemplateColumns:'minmax(0, 1.45fr) minmax(300px, .55fr)', gap:14, alignItems:'start' }}>
      <ExecutiveSection testId='top-governance-risks' title='Top Governance Risks' description='Priority queue for leadership focus across risk exposure, accountability, renewal urgency, savings and evidence.'>
        {data.topRisks.length === 0 ? <p style={{ color:'var(--text-secondary)', fontSize:13 }}>No governance risks recorded yet for this tenant.</p> : <div style={{ overflowX:'auto' }}><div style={{ minWidth:920 }}>
    ...
    <ExecutiveSection testId='domain-breakdown' title='Risk by Domain' description='Economic exposure first, with issue counts as secondary context.'>
      {data.domainBreakdown.length === 0 ? <p style={{ color:'var(--text-secondary)', fontSize:13 }}>No domain breakdown available yet.</p> : <>
