...
const pill = (status: string): PillStatus => status === 'degraded' ? 'degraded' : status === 'blocked' ? 'blocked' : status === 'testing' ? 'testing' : 'ready'

export default function ConnectorOperationsPage() {
  const workspace = useWorkspace()
  const { data, isEmptyLive, error, refresh } = useConnectorOpsData()
  ...
  const selectedConnector = data.connectors.find((connector:any) => connector.id === selected)
