...
export default function Recommendations() {
  ...
  return <Layout><div className="space-y-4"><h1 className="text-2xl font-semibold">Recommendations Control Plane</h1>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <Card className="p-4"><div className="space-y-3">{rows.map((r)=><div key={r.recommendationId} className="border rounded p-3 cursor-pointer" onClick={()=>setSelected(r)}><div className="flex justify-between"><div>{r.targetEntityId}</div><Badge>{wording[r.recommendationState] ?? r.recommendationState}</Badge></div><div className="text-xs text-muted-foreground">{playbookLabel(String(r.playbookId))} · Risk {r.actionRiskClass} · Evidence-backed {Array.isArray(r.evidencePointers)?r.evidencePointers.length:0}</div><div className="text-sm">${Number(r.projectedMonthlySavings).toFixed(2)}/mo · ${Number(r.projectedAnnualSavings).toFixed(2)}/yr</div><div className="text-xs">{Array.isArray(r.blockedReasons)&&r.blockedReasons.length?"Blocked by policy: "+r.blockedReasons.join(", "):"Savings visible, execution restricted"}</div></div>)}</div></Card>
