...
export default function CommandView({ params }: { params?: { domain?: string } }) {
  ...
  return <Shell><div style={{padding:'16px 20px'}}><h1>Command</h1>{notice && <div role='status' style={{border:'var(--border-default)',padding:8}}>{notice}</div>}{submitError && <div role='alert' style={{border:'var(--border-default)',padding:8}}>{submitError}</div>}<DomainTabs connectors={connectors} currentDomain={'all' as any} basePath='/:domain/command' />
    ...
    {isEmptyLive ? <EmptyState title='No actions identified yet' description='Connect your first data source to begin discovering optimisation opportunities.' ctaLabel='Connect data source →' /> : (
      <div>{error && <LiveDataError error={error} onRetry={refresh} />}<section data-testid='verification-watchlist' style={{border:'var(--border-default)',borderRadius:10,padding:12,marginTop:16,background:'var(--bg-card)'}}><SectionLabel>Verification Watchlist</SectionLabel><div>{data.metrics.pendingVerification ?? 0} outcomes awaiting verification</div><div>{money(data.metrics.projectedValuePendingProof ?? data.metrics.pendingVerification * 3733)} projected value pending proof</div><div>{data.metrics.failedVerification ?? 0} verification failure</div></section><section data-testid='command-live-activity' style={{border:'var(--border-default)',borderRadius:10,padding:12,marginTop:16,background:'var(--bg-card)'}}><SectionLabel>Live activity</SectionLabel><RuntimeActivityList events={latestEvents} limit={5} compact emptyLabel={activityError ? 'Runtime activity unavailable' : 'No runtime activity yet'} /></section>
