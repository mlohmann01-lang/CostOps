...
export default function TenantReadiness() {
  ...
  return <Shell><div style={{ padding: 24, display: 'grid', gap: 18 }}>
    <header style={{ display: 'grid', gap: 10 }}>
      ...
      {data.error && data.dataState !== 'NOT_CONNECTED' && <div role='alert' style={{ border: 'var(--border-amber)', background: 'var(--amber-bg)', borderRadius: 10, padding: 12 }}>Readiness data is unavailable: {data.error}</div>}
    ...
    <section style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0, 1fr))', gap: 10 }}>
      <Card><SectionLabel>Connector Status</SectionLabel><div style={{ marginTop: 8 }}><Badge label={data.connectorStatus.replace('_', ' ')} tone={connectorTone(data.connectorStatus)} /></div></Card>
      <Card><SectionLabel>Discovery Status</SectionLabel><div style={{ marginTop: 8 }}><Badge label={data.discoveryStatus.replace('_', ' ')} tone={discoveryTone(data.discoveryStatus)} /></div></Card>
      <Card><SectionLabel>Trust Status</SectionLabel><div style={{ marginTop: 8 }}><Badge label={data.trustStatus} tone={trustTone(data.trustStatus)} /></div></Card>
      <Card><SectionLabel>Recommendation Status</SectionLabel><div style={{ marginTop: 8 }}><Badge label={data.recommendationStatus} tone={recommendationTone(data.recommendationStatus)} /></div></Card>
      <Card><SectionLabel>Execution Readiness</SectionLabel><div style={{ marginTop: 8 }}><Badge label={data.executionReadiness} tone={executionTone(data.executionReadiness)} /></div></Card>
    </section>

    <section style={{ display: 'grid', gridTemplateColumns: 'repeat(1, minmax(0, 1fr))', gap: 10 }}>
      <MetricCard label='First Outcome Readiness' value={`${data.firstOutcomeReadinessPercent}%`} delta={data.requiredActions.length ? `${data.requiredActions.length} required action${data.requiredActions.length === 1 ? '' : 's'} remaining` : 'No blocking actions remaining'} hero />
    </section>
    ...
    <Card>
      ...
      {data.nextActions.length === 0 ? <p>No next actions. Onboarding is complete.</p> : <div style={{ display: 'grid', gap: 8, marginTop: 10 }}>
