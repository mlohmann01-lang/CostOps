...
const pill = (status: string): PillStatus => status === 'degraded' ? 'degraded' : status === 'pending' ? 'pending' : status === 'blocked' ? 'blocked' : status === 'active' ? 'active' : 'ready'

export default function RuntimeHealthView() {
  const { data, isEmptyLive } = useRuntimeHealthData()
  ...
  return <Shell><div style={{ padding: 20, display: 'grid', gap: 16 }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0, 1fr))', gap: 10 }} data-testid='runtime-component-grid'>
        {data.components.map((component) => <div key={component.id} style={{ border: 'var(--border-default)', background: 'var(--bg-card)', borderRadius: 10, padding: 12 }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
      <section style={{ border: 'var(--border-default)', borderRadius: 10, padding: 12 }}><SectionLabel>Active issues</SectionLabel>{data.activeIssues.map((issue) => <div key={issue.id} style={{ padding: '10px 0', borderTop: 'var(--border-subtle)' }}><StatusPill status={pill(issue.severity)} /> <strong>{issue.title}</strong><div style={{ color: 'var(--text-secondary)', fontSize: 12 }}>{issue.owner} · {issue.nextStep}</div></div>)}</section>
      <section style={{ border: 'var(--border-default)', borderRadius: 10, padding: 12 }}><SectionLabel>Recent events</SectionLabel>{data.recentEvents.map((event) => <div key={event.id} style={{ padding: '10px 0', borderTop: 'var(--border-subtle)' }}><span style={{ color: 'var(--text-tertiary)', fontSize: 12 }}>{event.at}</span> <strong>{event.event}</strong><div style={{ color: 'var(--text-secondary)', fontSize: 12 }}>{event.detail}</div></div>)}</section>
    </div>
  </div></Shell>
}
